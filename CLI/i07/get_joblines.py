...
def splitandopen(f, out):
    ...
    with (
        open(exp_file_path, "w") as text_file_exp,
...
if __name__ == "__main__":
    HELP_STR = "Takes in path to hdf5 output file, and shows exp_setup and calc_setup files used for processing. Opens up both setting in separate text files for viewing "

    parser = argparse.ArgumentParser(description=HELP_STR)
    HELP_STR = "Path to hdf5 file "
    parser.add_argument("-hf", "--hf_path", help=HELP_STR)

    HELP_STR = "Path to output directory to save example exp_setup and calc_setup files, defaults to home/fast_rsm"
