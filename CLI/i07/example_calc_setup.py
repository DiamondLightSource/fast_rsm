...
if no_nexus_experiment==False:
    ...
    if ('pyfai_qmap' in process_outputs)&(map_per_image==False)&(no_nexus_experiment==False):
    ...
    if ('pyfai_ivsq' in process_outputs)&(map_per_image==False)&(no_nexus_experiment==False):
        scanlist=experiment.scans
        name_end=scan_numbers[0]
        datetime_str = datetime.now().strftime("%Y-%m-%d_%Hh%Mm%Ss")
        projected_name=f'IvsQ_{name_end}_{datetime_str}'
    ...
    if ('pyfai_exitangles' in process_outputs)&(map_per_image==True)&(no_nexus_experiment==False):
        for i, scan in enumerate(experiment.scans):
            name_end=scan_numbers[i]
            datetime_str = datetime.now().strftime("%Y-%m-%d_%Hh%Mm%Ss")
    ...
    if ('pyfai_exitangles' in process_outputs)&(map_per_image==False)&(no_nexus_experiment==False):
        scanlist=experiment.scans
        name_end=scan_numbers[0]
        datetime_str = datetime.now().strftime("%Y-%m-%d_%Hh%Mm%Ss")
        projected_name=f'exitmap_{name_end}_{datetime_str}'
    ...
    if ('full_reciprocal_map' in process_outputs)&(no_nexus_experiment==False):
        frame_name = Frame.hkl
        coordinates = Frame.cartesian
        map_frame = Frame(frame_name=frame_name, coordinates=coordinates)
        start_time = time()
    ...
    #assume for now this will only be needed for single image cases, so map_per_image will always be true
    if ('pyfai_qmap' in process_outputs)&(map_per_image==True):
