...
def create_process_config(
    ...
    cfg.dps_centres = [
        cfg.dpsx_central_pixel,
        cfg.dpsy_central_pixel,
        cfg.dpsz_central_pixel,
        cfg.dpsz2_central_pixel,
    ]
    cfg.oop = initial_value_checks(
        cfg.dps_centres, cfg.cylinder_axis, cfg.setup, cfg.output_file_size
    )
...
def data_corruption_warning(cfg: SimpleNamespace):
    ...
    print(
        colour_text(
            "red",
            "WARNING - the following .h5 files were found to be corrupt\
...
def create_experiment(process_config: SimpleNamespace):
    ...
    experiment = Experiment.from_i07_nxs(
        cfg.good_nxs_paths,
        cfg.beam_centre,
        cfg.detector_distance,
        cfg.setup,
        cfg.local_data_path,
        using_dps=cfg.using_dps,
    ...
    experiment, cfg = standard_adjustments(experiment, cfg)
...
def initial_value_checks(dps_centres, cylinder_axis, setup, output_file_size):
    ...
    returns: oop
    """
    dpsx_central_pixel, dpsy_central_pixel, dpsz_central_pixel, dpsz2_central_pixel = (
        dps_centres
    )
    # Warn if dps offsets are silly.
    if (
        (dpsx_central_pixel > 10)
        or (dpsy_central_pixel > 10)
        or (dpsz_central_pixel > 10)
        or (dpsz2_central_pixel > 10)
    ):
        raise ValueError(
            "DPS central pixel units should be meters. Detected values greater than 10m"
        )
    ...
    if output_file_size > 2000:
        raise ValueError(
            "output_file_size must not exceed 2000. "
...
def setup_dps(scan, process_config):
    ...
    if scan.metadata.data_file.setup == "DCD":
        ...
        lab_frame = Frame(
            Frame.lab, scan.metadata.diffractometer, coordinates=Frame.cartesian
        )
        beam_direction = scan.metadata.diffractometer.get_incident_beam(lab_frame).array
    ...
    for i, scan in enumerate(experiment.scans):
        ...
        if cfg.skipscans is not None:
            if int(cfg.scan_numbers[i]) in cfg.skipscans:
                scan.skip_images += cfg.skipimages[
                    np.where(np.array(cfg.skipscans) == int(cfg.scan_numbers[i]))[0][0]
                ]
...
def make_mask_lists(specific_pixels, mask_regions):
    ...
    if mask_regions is not None:
        mask_regions_list = [
            maskval if isinstance(maskval, Region) else Region(*maskval)
            for maskval in mask_regions
        ]
        mask_regions = [
            [val.x_start, val.x_end, val.y_start, val.y_end]
            if isinstance(val, Region)
            else val
            for val in mask_regions
        ]
...
def make_new_hdf5(
    cfg: SimpleNamespace, scan_index: int, name_start: str, experiment: Experiment
):
    """
...
def run_full_map_process(experiment, cfg):
    ...
    cfg.mapped_data = experiment.binned_reciprocal_space_map_smm(
        ...
        map_frame,
        cfg,
        output_file_size=cfg.output_file_size,
        oop=cfg.oop,
        min_intensity_mask=cfg.min_intensity,
        output_file_name=save_path,
        volume_start=cfg.volume_start,
        volume_stop=cfg.volume_stop,
...
def save_binoviewer_hdf5(output_path: str, process_config: SimpleNamespace):
    ...
    binoviewer_start_int = [int(np.floor(start[i] / step[i])) for i in range(3)]
    binoviewer_stop_int = [
        int(binoviewer_start_int[i] + volume.shape[i] - 1) for i in range(3)
    ]
    binoviewer_stop = [binoviewer_stop_int[i] * binoviewer_step[i] for i in range(3)]

    # Make h, k and l arrays in the expected format.
    x_arr, y_arr, z_arr = tuple(
        np.array(
            [
                i,
                binoviewer_start[i],
