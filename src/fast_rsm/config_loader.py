...
def validate_beam_centre(bc):
    ...
    if np.shape(bc) != (2,):
        raise ValueError(
            f"beam_centre not correct shape. \
...
def validate_spherical_bragg(bragg):
    ...
    if np.shape(bragg) != (3,):
        raise ValueError(
            f"bragg vector not correct shape. \
...
def check_node_list(nodevalue, assignments):
    ...
    try:
        ...
        if all([len(item.strip()) == 0 for item in items_stripped]):
            return None
        outvals = [
            simp_eval.eval(item.strip()) if (len(item) > 0) else None
