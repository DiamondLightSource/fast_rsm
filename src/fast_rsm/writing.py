...
def linear_bin_to_vtk(binned_data: np.ndarray,
    ...
    """
    # Coordinates
    x_range = np.arange(start[0], stop[0] + step[0], step[0], dtype="float32")
    y_range = np.arange(start[1], stop[1] + step[1], step[1], dtype="float32")
    z_range = np.arange(start[2], stop[2] + step[2], step[2], dtype="float32")
