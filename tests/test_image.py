...
def test_get_coordchange():
    yvals = np.array([[1, 0, 0], [0, 0, -1], [0, 1, 0]])
    xvals = np.array([[0, 1, 0], [0, 0, 1], [1, 0, 0]])
    zvals = np.array([[1, 0, 0], [0, 1, 0], [0, 0, 1]])
...
def test_normvalue():
    ...
    testvals3 = None
    assert get_norm_value(testvals3, 2) is None
