...
from fast_rsm.config_loader import (
    ...
    experiment_config,
    parse_setup_file,
    validate_beam_centre,
    validate_outputs,
    validate_spherical_bragg,
)
...
def test_spherical_bragg():
    bragg_vec1 = [1, 2, 3]
    bragg_vec2 = [1.2]
    assert validate_spherical_bragg(bragg_vec1)
...
def test_validate_beamcentre():
    bc1 = [100, 20]
    bc2 = [12]
    assert validate_beam_centre(bc1)
    with pytest.raises(ValueError):
        validate_beam_centre(bc2)


def test_validateoutputs():
    outputs1 = ["full_reciprocal_map", "pyfai_ivsq", "pyfai_qmap", "pyfai_exitangles"]
    output2 = "pyfai_1D"
    output3 = "Nonesens"
    with pytest.raises(SchemaError):
        validate_outputs(output3)
    assert validate_outputs(outputs1)
    with pytest.raises(SchemaError):
...
def test_experimentconfig():
    ...
    assert check_config_schema(testdict)
