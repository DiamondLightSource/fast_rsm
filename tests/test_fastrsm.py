...
def test_process_script(exp_setup_path, scans):
    print(f"Running test with setup file: {exp_setup_path} and scans: {scans}", flush=True)
    returncode= run_process(exp_setup_path, scans)
    
    
    print(f"finished testing  setup file: {exp_setup_path} and scans: {scans}", flush=True)
